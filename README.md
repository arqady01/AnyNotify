...
AnyNotify 的事件摘要、任务状态和诊断记录保存在 `~/Library/Application Support/AnyNotify/`，不包含完整提示词或代码内容。日志读取游标单独持久化，用于应用重启后续读；菜单栏的“清空本地记录”不会删除 Claude/Codex 原始日志或游标。事件记录自动保留最近 30 天，最多 10,000 条。

完成提醒悬浮窗会保持置顶。提醒时长可在菜单栏中设置为 1–60 分钟，默认 3 分钟。倒计时归零后显示“已超时 x 秒”，并逐秒累计；如果倒计时期间又检测到新的完成事件，会立即按当前设置重新开始。
