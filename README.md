...
倒计时剩余 30 秒时会连续播放三次急促的系统提示音，归零时会再次播放一次提示音。手动关闭提醒或检测到新任务开始后，尚未触发的倒计时声音会被取消。
...
- 会话日志只在本机解析；
- 没有网络请求或远程数据上传；
- 通知正文可能包含最终回复的第一条有效文本；
- 最近状态只保存在内存中，应用退出后会清空；
- Claude Hooks 只有在用户点击安装按钮后才会写入配置。

如果不希望锁屏通知显示任务摘要，可以在 macOS 通知设置中关闭 AnyNotify 的通知预览。
